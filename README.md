...
This project implements a caching system for Pokemon data using a Least Recently Used (LRU) cache mechanism. It also provides a RESTful API to interact with the cache, allowing users to add, retrieve, and delete Pokémon data.

## Features

- **Cache Implementation**: LRU Cache with a maximum capacity.
- **REST API**:
 - Get Pokémon data by ID
 - Get Pokémon data by Name
 - Add new Pokémon data
 - Delete Pokémon data by ID
